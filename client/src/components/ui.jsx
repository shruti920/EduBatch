...
export const PageHeader = ({ title, documentTitle, description, actions }) => {
  ...
  return (
    <div className="flex flex-col gap-4 border-b border-paper-border pb-5 sm:flex-row sm:items-end sm:justify-between">
      <div>
...
export const Section = ({ title, aside, children, className = "" }) => (
  <section className={`min-w-0 rounded-md border border-paper-border bg-white ${className}`}>
    {(title || aside) && (
...
/**
 * A single bordered strip of figures, read left to right like a mark-sheet row.
 * items: [{ label, value, note, tone }] — works for 2 to 4 items.
 */
const STRIP_COLS = { 2: "sm:grid-cols-2", 3: "sm:grid-cols-3", 4: "grid-cols-2 lg:grid-cols-4" };

export const StatStrip = ({ items }) => (
  <dl
    className={`grid gap-px overflow-hidden rounded-md border border-paper-border bg-paper-border ${
      STRIP_COLS[items.length] || STRIP_COLS[4]
    }`}
  >
    {items.map((item) => (
      <div key={item.label} className="bg-white px-4 py-4">
        ...
        <dd
          className={`mt-1 text-2xl font-semibold tabular-nums ${
            item.tone === "attention" ? "text-attention" : item.tone === "success" ? "text-forest" : "text-ink"
          }`}
        >
          {item.value}
        </dd>
...
export const Pill = ({ tone = "neutral", children }) => (
  <span
    className={`inline-flex items-center rounded border px-2 py-0.5 text-xs font-medium whitespace-nowrap ${PILL_TONES[tone]}`}
  >
    {children}
  </span>
);
...
const BUTTON_VARIANTS = {
  primary: "bg-ink text-white hover:bg-ink-700 disabled:bg-ink-500",
  secondary: "bg-white text-ink border border-paper-border hover:border-ink",
...
export const Button = ({ variant = "primary", className = "", type = "button", ...props }) => (
  <button
    type={type}
    className={`inline-flex items-center justify-center gap-2 rounded px-3.5 py-2 text-sm font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${BUTTON_VARIANTS[variant]} ${className}`}
    {...props}
  />
);
...
export const Loading = ({ label = "Loading…" }) => (
  <div className="flex items-center gap-2 px-4 py-10 text-sm text-ink-muted" role="status">
    <span className="h-4 w-4 animate-spin rounded-full border-2 border-ink border-t-transparent motion-reduce:animate-none" />
    {label}
  </div>
);
...
export const inputClass =
  "w-full rounded border border-paper-border bg-white px-3 py-2 text-sm text-ink-text placeholder:text-ink-muted/70 focus:border-ink focus:outline-none disabled:bg-paper-muted";
...
export const Modal = ({ open, title, onClose, children, footer, size = "md" }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-ink-900/50 p-4 sm:items-center">
      ...
      <div
        ...
        tabIndex={-1}
        className={`relative my-8 w-full rounded-md border border-paper-border bg-white shadow-xl focus:outline-none ${
