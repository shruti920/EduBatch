...
const testRoleGuard = async () => {
  ...
  try {
    ...
    console.assert(res1.status === 403, `Expected 403 for student on admin route, got ${res1.status}`);
    const data1 = await res1.json();
    console.assert(data1.success === false, "Expected success: false");
    ...
    console.assert(res2.status === 403, `Expected 403 for teacher on admin route, got ${res2.status}`);
    ...
    console.assert(res3.status === 200, `Expected 200 for admin on admin route, got ${res3.status}`);
    ...
    console.assert(res4.status === 200, `Expected 200 for teacher on faculty route, got ${res4.status}`);
