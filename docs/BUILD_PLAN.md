...
- [x] Create `client/` (`npm create vite@latest client -- --template react`) and `server/` folders in one repo
- [x] `server/`: `npm init -y`, install `express mongoose dotenv cors helmet bcrypt jsonwebtoken zod express-rate-limit`
- [x] `client/`: install `tailwindcss axios react-router-dom`, configure Tailwind
- [x] Add root `.gitignore` — `node_modules`, `.env`, `dist`, `.DS_Store`
- [x] Add `server/.env.example` with placeholder keys (`MONGO_URI`, `JWT_SECRET`, `RAZORPAY_KEY_ID`, etc.) — never the real values
- [x] `server/src/config/db.js` — Mongoose connection, no models yet
- [x] `server/src/app.js` — Express app with `helmet()`, `cors()`, JSON body parser, a `/health` route that returns `{ ok: true }`
...
- [x] `models/User.js`
- [x] `utils/hash.js` — bcrypt hash/compare helpers
- [x] `utils/jwt.js` — sign/verify access token
- [x] `middleware/auth.js` — `protect` (verifies JWT, attaches `req.user`) and `restrictTo(...roles)`
- [x] `controllers/authController.js` + `routes/auth.js` — `POST /register`, `POST /login`, `GET /me`
- [x] Zod schemas for register/login payloads
- [x] Frontend: `AuthContext`, axios instance with token interceptor
- [x] Frontend: Register + Login pages, `ProtectedRoute` wrapper that checks role
- [x] Frontend: three empty dashboard shells (`/admin`, `/teacher`, `/student`) that a logged-in user lands on based on role
...
- [x] `models/Batch.js`
- [x] `controllers/batchController.js` + `routes/batches.js` — `GET /batches` (role-filtered), `GET /batches/:id`, `POST /batches` (admin), `PUT /batches/:id` (admin), `PATCH /batches/:id/status` (admin)
- [x] Zod schema for batch create/update
- [x] Frontend: batch list (table for admin, read-only cards for teacher/student), create/edit batch form (admin only)
...
- [x] `models/Enrollment.js`
- [x] `controllers/enrollmentController.js` + `routes/enrollments.js` — `POST /enrollments` (admin, **capacity check here**: count active enrollments for the batch against `batch.capacity` before inserting), `GET /enrollments/my` (student)
- [x] Frontend: "Enroll student" action on batch detail page (admin)
- [x] Frontend: "My batches" view (student) pulling from `/enrollments/my`
...
- [x] `controllers/dashboardController.js` + `routes/dashboard.js` — `/dashboard/admin` (total students, active batches, pending fees), `/dashboard/teacher` (assigned batches, student counts), `/dashboard/student` (enrolled batches, fee status)
- [x] Frontend: wire the three dashboard shells from Module 1 to real numbers
...
- [x] `models/Payment.js`
- [x] `config/razorpay.js` — SDK init with test keys from `.env`
- [x] `controllers/paymentController.js` + `routes/payments.js` — `POST /payments/create-order`, `POST /payments/verify` (**signature check happens server-side here — this is the one step that can't be skipped under time pressure**)
- [x] On verify success: update `Payment.status` and `Enrollment.paymentStatus`
- [x] Frontend: "Pay now" button → Razorpay Checkout modal → verify call
- [x] Frontend: payment history list

**Checkpoint:** The automated payment suite passed using local Razorpay SDK stand-ins and checks server-side signatures, webhooks, order reuse, and idempotency. A real Razorpay test-mode Checkout transaction has not been run; it needs test keys and a separate Checkout session.
...
- [x] `models/Attendance.js`
- [x] `controllers/attendanceController.js` + `routes/attendance.js` — `POST /attendance` (teacher, own batch only), `GET /attendance/batch/:id`, `GET /attendance/my` (student %)
- [x] Frontend: attendance grid for teacher (mark whole class in one submit), attendance % view for student
...
- [x] `models/Notice.js`
- [x] `controllers/noticeController.js` + `routes/notices.js` — `POST /notices` (teacher/admin), `GET /notices` (filter by batch)
- [x] Frontend: notice feed on student dashboard, create-notice form for teacher
